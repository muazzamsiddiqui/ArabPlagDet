...
public class SubString {
    ...
    private final double thresholdDocuments = 0.15;
    private final double thresholdSentences = 0.15;

    public SubString(ProcessingFile processingFile) {
            ...
            for (int i = 0; i < processingFile.getListOfSentences().size(); i++) {
                ...
                for (int j = 0; j < listOfSentences2.size(); j++) {
                    ...
                    if (temp >= thresholdSentences) {
                        ...
                        repeatedSubString1 = new ArrayList<List<Integer>>();
                        repeatedSubString2 = new ArrayList<List<Integer>>();
                        countAdd = 0;
                        ...
                        commonProperty.setRepeatedSubString1(repeatedSubString1);
                        commonProperty.setRepeatedSubString2(repeatedSubString2);
                        returnAllCommonSubString(ListOFWords1, ListOFWords2, commonProperty);
                    }
                }
            }
    ...
    private List<StringProperty> getStringProperties() {
        ...
        for (int k = 0; k < AllComoon.size(); k++) {
            ...
            //System.out.println("temp:"+tempWords);
            //tempInfo = i + " " + j + " " + AllComoon.get(k).get(1) + " [";

            /*
            for (int ikk = 0; ikk < commonProperty.getRepeatedSubString1().get(k).size(); ikk++) {
                //tempInfo += " " + repeatedSubString1.get(k).get(ikk);
                stringProperty.AddToCopiedFileIndexList(commonProperty.getRepeatedSubString1().get(k).get(ikk).toString());
            }
            //tempInfo += " ]  [";

            for (int ikk = 0; ikk < commonProperty.getRepeatedSubString2().get(k).size(); ikk++) {
                //tempInfo += " " + repeatedSubString2.get(k).get(ikk);
                stringProperty.AddToSourceIndexList(commonProperty.getRepeatedSubString2().get(k).get(ikk).toString());
            }
            //tempInfo += " ] ";
            */
    ...
    private void returnAllCommonSubString(List<String> Liststr1, List<String> Liststr2, CommonProperty commonProperty) {
        String str1 = Util.getConcatenatedString(Liststr1) + " ";
        String str2 = Util.getConcatenatedString(Liststr2) + " ";
        while (true) {
            List<String> temp = Util.longestSubstring(Liststr1, Liststr2, 3);
            if (temp == null)
                break;
            ...
            try {
                returnAllCommonSubString(Arrays.asList(str1.split("\\s+")), Arrays.asList(str2.split("\\s+")), commonProperty);
            } catch (Exception e) {
                e.printStackTrace();
            }
